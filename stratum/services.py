...
def admin(func):
    ...
    def inner(*args, **kwargs):
        ...
        if config.ADMIN_RESTRICT_INTERFACE != None:
            ip = args[0].connection_ref()._get_ip()
            if config.ADMIN_RESTRICT_INTERFACE != ip:
                raise custom_exceptions.UnauthorizedException("RPC call not allowed from your IP")

        if not config.ADMIN_PASSWORD_SHA256:
        ...
        if hashlib.sha256(password).hexdigest() != config.ADMIN_PASSWORD_SHA256:
