...
class Protocol(LineOnlyReceiver):
    ...
    def connectionMade(self):
        ...
        # Read config.TCP_PROXY_PROTOCOL documentation
        ...
        try:
            # When we're not expecting PROXY line, pass the information about the connection immediately
            if not self.expect_tcp_proxy_protocol_header:
                self.event_handler._handle_event('<connected>', (), connection=self)
        except:
            pass
    ...
    def connectionLost(self, reason):
        ...
        try:
            self.event_handler._handle_event('<disconnected>', (), connection=self)
        except:
            pass
    ...
    def lineReceived(self, line, request_counter):
        if self.expect_tcp_proxy_protocol_header:
            ...
            if line.startswith('PROXY'):
                ...
                # Pass information about just made connection to the handler. We waited with
                # passing the event to a time when real IP address is known.
                self.event_handler._handle_event('<connected>', (), connection=self)
                return
            else:
                log.warning("PROXY line not found")
                self.proxied_ip = '0.0.0.0'
                self.event_handler._handle_event('<connected>', (), connection=self)
        ...
        if msg_method:
            ...
            try:
                result = self.event_handler._handle_event(msg_method, msg_params, connection=self)
